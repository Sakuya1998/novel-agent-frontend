import { apiOperation } from "../../api/client";
...
export async function listResources(
  ...
  const response = (await apiOperation(operations.list, {
    parameters: { path: { workspace_id: workspaceId, resource_kind: kind }, query },
  } as never)) as ListResponse;
...
export function createResource(
  workspaceId: string,
  kind: ResourceKind,
  body: { key: string; name: string; description: string; payload?: Record<string, unknown> },
) {
  return apiOperation(operations.create, {
    parameters: { path: { workspace_id: workspaceId, resource_kind: kind } },
    body,
  } as never) as Promise<Resource>;
}
export function updateResource(
  workspaceId: string,
  kind: ResourceKind,
  id: string,
  body: Partial<Resource> & { expected_version?: number },
) {
  return apiOperation(operations.update, {
    parameters: { path: { workspace_id: workspaceId, resource_kind: kind, resource_id: id } },
    body,
  } as never) as Promise<Resource>;
}
export function listResourceVersions(workspaceId: string, kind: ResourceKind, id: string) {
  return apiOperation(operations.versions, {
    parameters: { path: { workspace_id: workspaceId, resource_kind: kind, resource_id: id } },
  } as never) as Promise<ListResponse>;
}
...
export function copyStyleResource(
  workspaceId: string,
  id: string,
  body: { key: string; name: string; description: string },
) {
  return apiOperation(operations.copy, {
    parameters: { path: { workspace_id: workspaceId, resource_id: id } },
    body,
  } as never) as Promise<Resource>;
}
