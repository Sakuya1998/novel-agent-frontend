...
export function ReviewWorkspace({
  ...
  const hasVersionCommands = Boolean(
    onCompareVersions || onEvaluateVersion || onSetEvaluationBaseline || onCompareEvaluations,
  );
  ...
  const evaluateVersion =
    onEvaluateVersion ??
    (async () => {
      throw new Error("章节评测不可用");
    });
  const setEvaluationBaseline =
    onSetEvaluationBaseline ??
    (async () => {
      throw new Error("设置基准不可用");
    });
  const compareEvaluations =
    onCompareEvaluations ??
    (async () => {
      throw new Error("回归比较不可用");
    });
  ...
  if (activeTab === "issues") {
    activeContent = (
      <ReviewIssuesPanel
        ...
        busyAction={activeBusyAction}
        onApplyCanon={
          onApplyCanon ? (operation) => workflow.runAction("canon", () => onApplyCanon(operation)) : undefined
        }
        onRepairFeedback={(feedback) => workflow.replaceDraft({ feedback })}
      />
    );
    ...
    activeContent = (
      <ChapterCandidatesPanel
        ...
        onGenerate={
          onGenerateCandidates
            ? (count, instruction) => workflow.runAction("generate", () => onGenerateCandidates(count, instruction))
            : undefined
        }
        onSelect={(candidateId) =>
          workflow.replaceDraft({ feedback: "candidate", candidate_id: candidateId }, returnToDecision)
        }
      />
    );
      ...
      <>
        {versions.length > 0 ? (
          <VersionHistory
            ...
            onRestore={(versionNumber) =>
              workflow.replaceDraft({ feedback: "restore", version_number: versionNumber }, returnToDecision)
            }
          />
        ) : (
        ...
        {hasEvaluationSurface ? (
          <ChapterEvaluationPanel
            ...
            disabled={
              disabled ||
              Boolean(activeBusyAction) ||
              !onEvaluateVersion ||
              !onSetEvaluationBaseline ||
              !onCompareEvaluations
            }
            onEvaluate={evaluateVersion}
            onSetBaseline={setEvaluationBaseline}
            onCompare={compareEvaluations}
          />
        ) : null}
      </>
    );
  ...
  return (
    <aside className="review-panel review-workspace">
      <header className="review-header">
        ...
        <div>
          ...
          <h2>第 {draft.chapter_number ?? "—"} 章审查</h2>
        </div>
        <span className="review-status">待处理</span>
      </header>
      ...
      <section
        ...
        className={`review-tab-panel ${activeTab === "decision" ? "has-decision-dock" : ""}`}
      >
        {activeTab !== "decision" && workspaceError ? (
