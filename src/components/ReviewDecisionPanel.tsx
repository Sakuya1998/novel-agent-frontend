...
export function ReviewDecisionPanel({
  ...
  return (
    <>
      {scenePlan.length > 0 ? (
        <div className="scene-plan-block">
          ...
          {scenePlan.map((scene) => (
            ...
            >
              ...
              <div>
                ...
                <p>
                  {scene.location} · {scene.emotion} · {scene.estimated_words} 字
                </p>
              </div>
            </button>
          ))}
      ...
      <div className="review-form review-decision">
        ...
        <div className="review-actions">
          ...
          >
            <Send size={14} />
            {busyAction === "revision" ? "重写中…" : sceneNumber === undefined ? "重写整章" : "重写此场景"}
          </button>
          <button type="button" className="primary-button" onClick={() => void onApprove()} disabled={controlsDisabled}>
            <Check size={15} />
            {busyAction === "approve" ? "定稿中…" : "通过定稿"}
          </button>
        </div>
      </div>
    </>
