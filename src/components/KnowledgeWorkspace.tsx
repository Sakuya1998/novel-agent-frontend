  ...
  return (
    <section className="workspace-view knowledge-workspace" aria-labelledby="knowledge-view-title">
      <header className="workspace-view-header">
        <div>
          <span className="section-label">STORY KNOWLEDGE</span>
          <h2 id="knowledge-view-title">设定与记忆</h2>
        </div>
        <span>版本化保存</span>
      </header>
      ...
      <div className="knowledge-action-list">
        <button type="button" onClick={onOpenBrief}>
          ...
          <span>
            <strong>创作约束</strong>
            <small>{brief?.target_audience || "设置目标读者、视角和内容边界"}</small>
          </span>
        ...
        <button type="button" onClick={onOpenCanon}>
          ...
          <span>
            <strong>事实与叙事线程</strong>
            <small>
              {facts} 条事实 · {state.canon?.open_threads ?? 0} 条开放线索
            </small>
          </span>
        ...
        <button type="button" onClick={onOpenMemory}>
          ...
          <span>
            <strong>长期记忆</strong>
            <small>
              {state.memory?.chapters ?? 0} 章记忆 · {state.memory?.arcs ?? 0} 个故事幕
            </small>
          </span>
