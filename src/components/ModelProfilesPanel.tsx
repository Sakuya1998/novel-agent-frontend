    ...
    <div className="model-profile-layout">
      <aside className="model-profile-list" aria-label="模型服务列表">
        ...
        {settings.profiles.length === 0 ? (
          <p className="model-settings-empty">尚未保存模型服务</p>
        ) : (
          settings.profiles.map((profile) => (
            ...
            >
              <span>
                ...
                <small>{settings.templates[profile.provider]?.label ?? profile.provider}</small>
              </span>
      ...
      <section className="model-profile-editor">
        ...
        <div className="model-form-grid">
          ...
          <label className="model-form-wide">
            ...
            <input
              value={baseUrl}
              disabled={formDisabled || baseUrlMode !== "required"}
              onChange={(event) => setBaseUrl(event.target.value)}
              placeholder={baseUrlMode === "hidden" ? "由供应商 SDK 管理" : "https://api.example.com/v1"}
            />
          </label>
          <label className="model-form-wide">
            API Key
            <input
              ...
              value={apiKey}
              disabled={formDisabled || clearApiKey}
              onChange={(event) => setApiKey(event.target.value)}
              placeholder={selected?.has_api_key ? "留空保留现有密钥" : apiKeyRequired ? "输入 API Key" : "可选"}
            />
            {selected?.has_api_key ? (
              <small className="configured-secret">已配置 · {selected.api_key_masked}</small>
            ) : (
              <small>{apiKeyRequired ? "密钥只会加密保存，不会从接口返回明文" : "此服务允许无密钥连接"}</small>
            )}
          </label>
          {selected?.has_api_key ? (
          ...
          <label>
            ...
            <textarea
              ...
              disabled={formDisabled || !supportsEmbeddings}
              onChange={(event) => setEmbeddingModels(event.target.value)}
            />
          </label>
        </div>
        ...
        <div className="model-editor-actions">
          <button
            ...
            type="button"
            disabled={formDisabled || !selected || parsedChatModels.length === 0}
            onClick={() => selected && void onTest(selected.id, "chat", parsedChatModels[0]).catch(() => undefined)}
          >
            {busyAction === "test-chat" ? <LoaderCircle className="spin" size={14} /> : <FlaskConical size={14} />}{" "}
            测试聊天模型
          </button>
          {supportsEmbeddings ? (
            <button
              className="secondary-button"
              type="button"
              disabled={formDisabled || !selected || parsedEmbeddingModels.length === 0}
              onClick={() =>
                selected && void onTest(selected.id, "embedding", parsedEmbeddingModels[0]).catch(() => undefined)
              }
            >
              {busyAction === "test-embedding" ? (
