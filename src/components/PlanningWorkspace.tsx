  ...
  return (
    <section className="workspace-view planning-workspace" aria-labelledby="planning-view-title">
      <header className="workspace-view-header">
        <div>
          <span className="section-label">STORY PLAN</span>
          <h2 id="planning-view-title">故事规划</h2>
        </div>
        <span>
          {outline.length} 章 · {characters.length} 位角色
        </span>
      </header>
      ...
      <div className="planning-columns">
          ...
          <div className="planning-character-list">
            {characters.map((character, index) => (
              <article key={`${text(character.name)}-${index}`}>
                ...
                <div>
                  <strong>{text(character.name) || "未命名角色"}</strong>
                  <small>{text(character.role) || text(character.personality) || "尚未设置角色定位"}</small>
                </div>
              </article>
            ))}
        ...
        <section>
          <div className="planning-summary-heading">
            <Map size={16} />
            <strong>{scenePlan.length ? "当前分镜" : "章节大纲"}</strong>
          </div>
            ...
            {scenePlan.length
              ? scenePlan.map((scene) => (
                  <article key={scene.scene_number}>
                    ...
                    <div>
                      <strong>{scene.goal || "未命名场景"}</strong>
                      <small>
                        {scene.location || "地点未定"} · {scene.emotion || "情绪未定"}
                      </small>
                    </div>
              ...
              : outline.map((chapter, index) => (
                  <article key={`${text(chapter.chapter)}-${index}`}>
                    ...
                    <div>
                      <strong>{text(chapter.title) || `第 ${index + 1} 章`}</strong>
                      <small>{text(chapter.summary) || "暂无章节摘要"}</small>
                    </div>
                  </article>
                ))}
