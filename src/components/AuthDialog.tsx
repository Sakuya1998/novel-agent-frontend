  ...
  return (
    <div className="model-settings-backdrop" onMouseDown={onBackdropMouseDown}>
      <section
        ref={dialogRef}
        tabIndex={-1}
        className="model-settings-dialog auth-dialog"
        role="dialog"
        aria-modal="true"
        ...
        {currentUser ? (
          <div className="auth-current">
            <div>
              ...
              <strong>{currentUser.tenant_name}</strong>
              <small>
                {currentUser.role} · {currentUser.username}
              </small>
            </div>
            <button type="button" className="secondary-button" disabled={busy} onClick={() => void logout()}>
              <LogOut size={14} />
              {busy ? "退出中" : "退出登录"}
            </button>
            {error && (
          ...
          <>
            ...
            >
              ...
              <button
                ...
                aria-label={mode === "login" ? "提交登录" : "提交注册"}
                disabled={busy}
              >
                {mode === "login" ? <LogIn size={14} /> : <UserPlus size={14} />}
                {busy ? "提交中" : mode === "login" ? "登录" : "创建工作区"}
              </button>
            </form>
          </>
        )}
