...
describe("PlanningReviewPanel", () => {
  it.each(["stage", "novel"])(
    "clears a previous submission error when the planning %s changes",
    async (changedScope) => {
      const props = {
  ...
  it("ignores a rejected submission that settles after its planning scope changes", async () => {
    ...
    await act(async () => {
      reject(new Error("Old pending scope failed"));
  ...
  it("keeps planning edits and exposes a rejected resume command", async () => {
    render(
      <PlanningReviewPanel
        ...
        scenePlan={[]}
        disabled={false}
        onSubmit={vi.fn().mockRejectedValue(new Error("resume unavailable"))}
      />,
    );
