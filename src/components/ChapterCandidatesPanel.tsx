...
export function ChapterCandidatesPanel({
  candidates,
  currentContent,
  disabled,
  onGenerate,
  ...
  const compared = useMemo(() => candidates.find((candidate) => candidate.id === compareId), [candidates, compareId]);
  ...
  return (
    <section className="candidate-panel">
        ...
        <div className="candidate-list">
          {candidates.map((candidate) => (
            <article className={`candidate-card ${candidate.status}`} key={candidate.id}>
              <header>
                <div>
                  <span>方案 {candidate.candidate_number}</span>
              ...
              <div className="candidate-actions">
                <button
                  ...
                  disabled={disabled || submitting || Boolean(selectingId)}
                >
                ...
                <button
                  ...
                  disabled={disabled || submitting || Boolean(selectingId) || candidate.status === "stale"}
                >
                  <Check size={14} />
                  {candidate.status === "stale" ? "需重新生成" : selectingId === candidate.id ? "正在采用" : "采用此稿"}
                </button>
              </div>
            </article>
          ))}
      ...
      {compared && (
        <div className="candidate-compare" role="dialog" aria-label="候选稿对比">
          <header>
            <strong>当前稿与方案 {compared.candidate_number}</strong>
            <button type="button" className="icon-button" aria-label="关闭候选稿对比" onClick={() => setCompareId("")}>
              <X size={15} />
            </button>
          </header>
          <div>
            ...
            <article>
              <span>候选稿 · {compared.overall_score.toFixed(1)} 分</span>
              <p>{compared.content}</p>
            </article>
          </div>
        </div>
      )}
