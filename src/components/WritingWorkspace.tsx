...
function WritingNextAction({
  ...
  if (state.status === "blueprint_review" || state.status === "scene_review") {
    return (
      <aside className="next-panel review-required-panel">
        <div className="section-kicker">REVIEW REQUIRED</div>
        <CheckCircle2 size={21} />
        <h2>规划等待确认</h2>
        <p>批准当前蓝图或分镜后，正文创作才会继续。</p>
        <button className="primary-button full-width" type="button" onClick={onOpenPlanning}>
          <span>前往审阅</span>
          <ArrowRight size={15} />
        </button>
      </aside>
  ...
  if (state.status === "completed" && state.book_audit) {
    return (
      <aside className="next-panel review-required-panel completed">
        <div className="section-kicker">MANUSCRIPT COMPLETE</div>
        <CheckCircle2 size={21} />
        <h2>全书已经完成</h2>
        <p>终审报告已生成，可以检查全书质量或发起返修。</p>
        <button className="secondary-button full-width" type="button" onClick={onOpenQuality}>
          <span>查看终审</span>
          <ArrowRight size={15} />
        </button>
      </aside>
  ...
  return (
    <aside className="next-panel writing-next-action" aria-label="写作进度">
      <div className="section-kicker">WRITING WORKSPACE</div>
      <h2>{state.status === "running" ? "正在展开正文" : "稿件阅读"}</h2>
      <p>
        {state.status === "running" ? "状态栏会持续更新后台创作进度。" : "启动创作后，正文和章节进度会在这里展开。"}
      </p>
    </aside>
  );
}
...
export function WritingWorkspace({
  ...
  const currentFocus =
    readerFocus?.novelId === novel.id && readerFocus?.chapterNumber === state.current_draft.chapter_number
      ? readerFocus
      : undefined;
      ...
      <div className="writing-workspace-grid">
        ...
        {state.status === "human_review" ? (
          <ReviewWorkspace
            ...
            onFocusReader={(sceneNumber) =>
              onReaderFocusChange({
                novelId: novel.id,
                chapterNumber: state.current_draft.chapter_number,
                sceneNumber,
                target: sceneNumber === undefined ? "top" : "scene",
                request: (readerFocus?.request ?? 0) + 1,
              })
            }
          />
        ) : (
