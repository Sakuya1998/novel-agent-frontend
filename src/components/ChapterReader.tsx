...
export function ChapterReader({
  ...
  useEffect(() => {
    ...
    const target =
      focusTarget === "top"
        ...
        : selectedSceneNumber === undefined
          ? undefined
          : sceneRefs.current.get(selectedSceneNumber);
  ...
  return (
    <section className="reader-panel">
      ...
      {hasDraft ? (
        <article className="manuscript" ref={manuscriptRef} tabIndex={-1}>
          <div className="manuscript-meta">
            <span>第 {draft.chapter_number ?? "—"} 章</span>
            <span className="meta-divider">/</span>
            <span>{status === "human_review" ? "待审查稿" : "当前稿件"}</span>
          </div>
          <h2>{draft.title || "未命名章节"}</h2>
          <div className="manuscript-stats">
            <span>
              <AlignLeft size={14} />
              {draft.word_count || (draft.content?.length ?? 0)} 字
            </span>
            <span>
              <Clock3 size={14} />
              实时生成
        ...
        <div className="chapter-list">
          {chapters.map((chapter) => (
            <article className="chapter-row" key={chapter.chapter_number}>
              ...
              <span className="chapter-words">{chapter.word_count || 0} 字</span>
            </article>
          ))}
        </div>
      ) : (
        <div className="reader-empty">
          ...
          <strong>稿件将在这里展开</strong>
          <span>启动创作后，世界观、章节和审查结果会按阶段出现。</span>
        </div>
      )}
    </section>
  );
