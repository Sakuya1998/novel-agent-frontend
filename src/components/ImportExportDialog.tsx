...
export function ImportExportDialog({ open, novelTitle, onClose, onExport, onImport }: Props) {
  ...
  async function exportBook() {
    ...
    setMessage("");
    try {
      const result = await onExport(format, format === "backup" ? exportPassword : "", { author, publisher, language });
  ...
  async function importBook() {
    ...
    setMessage("");
    try {
      await onImport(file, importTitle.trim(), importPassword);
      setMessage("导入完成，作品列表已更新");
      setFile(undefined);
      setImportTitle("");
      setImportPassword("");
  ...
  return (
    <div className="model-settings-backdrop" onMouseDown={onBackdropMouseDown}>
      <section
        ref={dialogRef}
        tabIndex={-1}
        className="model-settings-dialog import-export-dialog"
        role="dialog"
        aria-modal="true"
        ...
        <div className="memory-quality-header">
          <div className="dialog-title">
            ...
            <div>
              ...
              <h2 id="import-export-title">导入与导出</h2>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="import-export-body">
            ...
            {novelTitle ? (
              <>
                <div className="import-export-row">
                  ...
                  <button type="button" className="primary-button" onClick={() => void exportBook()} disabled={busy}>
                    <Download size={14} />
                    导出文件
                  </button>
                </div>
                ...
                {format === "backup" && (
                  <label className="import-export-password">
                    备份密码
                    <input
                      aria-label="导出备份密码"
                      type="password"
          ...
          <section>
            ...
            <input
              ...
              aria-label="选择导入文件"
              type="file"
              accept=".md,.markdown,.txt,.docx,.epub,.zip,.novel-backup.zip,.enc"
            ...
            <button
              type="button"
              className="secondary-button import-submit"
          ...
          {message && (
            <p className="import-export-message" role="status">
              {message}
            </p>
          )}
        </div>
