    ...
    <div className="version-history">
      ...
      <div className="version-list">
        {versions.map((version) => (
          <div className="version-row" key={version.version_number}>
            <span>v{version.version_number}</span>
            <div>
              <strong>{SOURCE_LABELS[version.source] ?? version.source}</strong>
              <small>{version.word_count} 字</small>
            </div>
            <button
              type="button"
              className="version-restore"
              title={`恢复 v${version.version_number}`}
              aria-label={`恢复 v${version.version_number}`}
              disabled={disabled}
              onClick={async () => {
                try {
                  await onRestore(version.version_number);
                  onRestored?.();
                } catch (reason) {
                  onError?.(reason);
                }
      ...
      {versions.length > 1 && (
        <div className="version-compare">
          ...
          >
            {versions.map((version) => (
              <option value={version.version_number} key={version.version_number}>
          ...
          <button
            ...
            aria-label="比较版本"
            onClick={compare}
            disabled={disabled || loading || fromVersion === toVersion}
          >
