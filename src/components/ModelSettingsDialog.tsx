...
export function ModelSettingsDialog({ open, isStreaming, onClose }: Props) {
  ...
  const { dialogRef, onBackdropMouseDown } = useDialogLifecycle<HTMLElement>(
    open,
    onClose,
    Boolean(modelSettings.busyAction),
  );
  ...
  const sourceLabel =
    settings?.source === "database" ? "工作台配置" : settings?.source === "environment" ? "环境配置回退" : "尚未配置";
      ...
      >
        <header className="model-settings-header">
          ...
          <div className="model-settings-header-actions">
            <span className={`model-source-badge ${settings?.source ?? "unconfigured"}`}>{sourceLabel}</span>
            <button
              className="dialog-close-button"
              type="button"
              title="关闭"
              aria-label="关闭模型设置"
              disabled={Boolean(modelSettings.busyAction)}
              onClick={onClose}
            >
              <X size={18} />
            </button>
        ...
        {modelSettings.error ? (
          <div className="model-settings-message error" role="alert" aria-live="assertive">
            <AlertCircle size={15} />
            {modelSettings.error}
          </div>
        ) : null}
        {modelSettings.notice ? (
          <div className="model-settings-message success" role="status" aria-live="polite">
            <CheckCircle2 size={15} />
            {modelSettings.notice}
          </div>
        ) : null}
        <div className="model-settings-content">
          {modelSettings.isLoading && !settings ? (
            <div className="model-settings-loading">
              <LoaderCircle className="spin" size={18} />
              加载模型设置
            </div>
          ) : null}
          {!modelSettings.isLoading && modelSettings.error && !settings ? (
            <div className="model-settings-load-error">
              <AlertCircle size={20} />
              <strong>模型设置加载失败</strong>
              <button className="secondary-button" type="button" onClick={() => void modelSettings.reload()}>
                <RefreshCw size={14} />
                重新加载
              </button>
            </div>
