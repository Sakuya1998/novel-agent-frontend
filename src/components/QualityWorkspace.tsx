    ...
    <section className="workspace-view quality-workspace" aria-labelledby="quality-view-title">
      <header className="workspace-view-header">
        <div>
          ...
          <h2 id="quality-view-title">质量与运行</h2>
        </div>
      ...
      <div className="quality-summary">
        <div>
          <span>当前质量分</span>
          <strong>{report ? report.overall_score.toFixed(1) : "--"}</strong>
          <small>{report ? `门槛 ${report.threshold.toFixed(1)}` : "尚未评测"}</small>
        </div>
        <div>
          <span>模型调用</span>
          <strong>{usage.successful_calls}</strong>
          <small>{usage.failed_attempts} 次失败</small>
        </div>
        <div>
          <span>累计 Tokens</span>
          <strong>{new Intl.NumberFormat("zh-CN", { notation: "compact" }).format(usage.total_tokens)}</strong>
          <small>{usage.fallback_attempts} 次降级</small>
        </div>
      </div>
      ...
      <div className="quality-action-list">
        <button type="button" onClick={onOpenMonitoring}>
          ...
          <span>
            <strong>运行与审计</strong>
            <small>服务、后台任务和审计日志</small>
          </span>
          <ArrowUpRight size={16} />
        </button>
        <button type="button" onClick={onOpenBenchmarks}>
          <FlaskConical size={17} />
          <span>
            <strong>质量基准</strong>
            <small>运行并比较回归评测</small>
          </span>
          <ArrowUpRight size={16} />
        </button>
        <button type="button" onClick={onOpenTraces}>
          <Activity size={17} />
          <span>
