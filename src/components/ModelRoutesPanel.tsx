...
export function ModelRoutesPanel({ settings, disabled, busyAction, onSave }: Props) {
  ...
  const valid = routeMeta.every(({ purpose }) => {
    ...
    return (
      ...
      (purpose === "embedding" || Boolean(route.fallback_profile_id) === Boolean(route.fallback_model_name?.trim()))
    );
  });
  ...
  return (
    <section className="model-routes-panel">
      ...
      <div className="model-route-list">
        {routeMeta.map(({ purpose, label, description }) => {
          const eligibleProfiles = settings.profiles.filter(
            (profile) => purpose !== "embedding" || supportsEmbeddings(settings, profile.provider),
          );
          const selectedProfile = profileMap.get(routes[purpose].profile_id);
          const modelOptions =
            purpose === "embedding" ? selectedProfile?.embedding_models : selectedProfile?.chat_models;
          ...
          return (
            <div className="model-route-row" key={purpose}>
              ...
              <div className="model-route-controls">
                <label>
                  主模型服务
                  <select
                    value={routes[purpose].profile_id}
                    disabled={formDisabled}
                    onChange={(event) => changeProfile(purpose, event.target.value)}
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setRoutes((current) => ({
                        ...current,
                        [purpose]: { ...current[purpose], model_name: event.target.value },
                      }))
                    }
                  />
                  <datalist id={listId}>
                ...
                {purpose !== "embedding" && (
                  <>
                    <label>
                      备用模型服务
                      <select
                        value={routes[purpose].fallback_profile_id ?? ""}
                        disabled={formDisabled}
                        onChange={(event) => changeFallbackProfile(purpose, event.target.value)}
                      >
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRoutes((current) => ({
                            ...current,
                            [purpose]: { ...current[purpose], fallback_model_name: event.target.value },
                          }))
                        }
                      />
                      <datalist id={fallbackListId}>
                        {(fallbackProfile?.chat_models ?? []).map((model) => (
                          <option value={model} key={model} />
                        ))}
                      </datalist>
                    </label>
      ...
      {settings.profiles.length === 0 ? (
        <div className="model-route-warning">请先在“模型服务”中新增服务档案。</div>
      ) : null}
      <div className="model-route-actions">
        <button
          className="primary-button"
          type="button"
          disabled={formDisabled || !valid}
          onClick={() => void onSave(routes).catch(() => undefined)}
        >
