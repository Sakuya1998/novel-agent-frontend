  ...
  return (
    <aside className="planning-review-panel">
      <header className="planning-review-header">
        ...
        <div>
          <span>{reviewNode === "blueprint_review" ? "全书蓝图" : "本章分镜"}</span>
          <h2>生成正文前审阅</h2>
        </div>
      </header>
      ...
      <div className="planning-review-body">
        ...
        {reviewNode === "blueprint_review" ? (
          <>
            ...
            <section className="planning-editor-section">
              <div className="section-heading">
                ...
                <button
                  ...
                  title="添加角色"
                  onClick={() => setCharacterRows((rows) => [...rows, { name: "", role: "" }])}
                >
                  <Plus size={14} />
                </button>
              </div>
              <div className="structured-list">
                {characterRows.map((character, index) => (
                  <div className="character-editor" key={index}>
                    <input
                      aria-label={`角色 ${index + 1} 姓名`}
                      value={text(character.name)}
                      placeholder="姓名"
                      onChange={(event) => updateCharacter(index, "name", event.target.value)}
                    />
                    <input
                      aria-label={`角色 ${index + 1} 定位`}
                    ...
                    <textarea
                      ...
                      value={text(character.personality)}
                      placeholder="性格与动机"
                      rows={2}
                      onChange={(event) => updateCharacter(index, "personality", event.target.value)}
                    />
                    <button
                      type="button"
                      className="icon-button compact danger"
                      title="删除角色"
                      onClick={() => setCharacterRows((rows) => rows.filter((_, itemIndex) => itemIndex !== index))}
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
            ...
            <section className="planning-editor-section">
              ...
              <div className="structured-list">
                {outlineRows.map((chapter, index) => (
                  <div className="outline-editor" key={text(chapter.chapter) || index}>
                    <span className="row-number">{text(chapter.chapter) || index + 1}</span>
                    <input
                      value={text(chapter.title)}
                      placeholder="章节标题"
                    ...
                    />
                    <input
                      value={text(chapter.conflict)}
                      placeholder="核心冲突"
                      onChange={(event) => updateOutline(index, "conflict", event.target.value)}
                    />
                    <input
                      value={text(chapter.cliffhanger)}
                      placeholder="章末悬念"
                      onChange={(event) => updateOutline(index, "cliffhanger", event.target.value)}
                    />
                    <input
                      type="number"
                      min="1"
                      value={Number(chapter.estimated_words) || 1}
                      aria-label={`第 ${index + 1} 章目标字数`}
                      onChange={(event) => updateOutline(index, "estimated_words", Number(event.target.value))}
                    />
                  </div>
                ))}
              </div>
          ...
          <section className="planning-editor-section">
            <div className="section-heading">
              ...
              <button
                ...
                className="icon-button compact"
                title="添加场景"
                disabled={sceneRows.length >= 8}
                onClick={() =>
                  setSceneRows((rows) => [
                    ...rows,
                    {
                      scene_number: rows.length + 1,
                      goal: "",
                      conflict: "",
                      turn: "",
                      location: "",
            ...
            <div className="structured-list">
              {sceneRows.map((scene, index) => (
                <div className="scene-editor" key={scene.scene_number}>
                  <span className="row-number">{index + 1}</span>
                  <input
                    value={scene.goal}
                    placeholder="场景目标"
                    onChange={(event) => updateScene(index, "goal", event.target.value)}
                  />
                  <input
                    value={scene.conflict}
                    placeholder="冲突"
                    onChange={(event) => updateScene(index, "conflict", event.target.value)}
                  />
                  <input
                    value={scene.turn}
                    placeholder="转折"
                    onChange={(event) => updateScene(index, "turn", event.target.value)}
                  />
                  <input
                  ...
                  <input
                    value={scene.emotion}
                    placeholder="情绪"
                    onChange={(event) => updateScene(index, "emotion", event.target.value)}
                  />
                  <input
                    type="number"
                    min="1"
                    value={scene.estimated_words}
                    aria-label={`场景 ${index + 1} 目标字数`}
                    onChange={(event) => updateScene(index, "estimated_words", Number(event.target.value))}
                  />
                  <button
                    type="button"
                    className="icon-button compact danger"
        ...
        {planningVersions.length > 0 && (
          <section className="planning-editor-section planning-version-section">
            <div className="section-heading">
              <strong>
                <History size={14} />
                版本历史
            ...
            <div className="planning-version-list">
              {planningVersions.map((version) => (
                <div className="planning-version-row" key={version.id}>
                  <span>v{version.version_number}</span>
                  <div>
                    <strong>
                      {version.source === "generated"
                        ? "生成稿"
                        : version.source === "approved"
                          ? "批准稿"
                          : version.source}
                    </strong>
            ...
            <div className="planning-version-controls">
              ...
              >
                {planningVersions.map((version) => (
                  <option value={version.version_number} key={version.id}>
              ...
              <button
                ...
                disabled={disabled || historyBusy || fromVersion === toVersion}
              ...
              <button
                type="button"
                className="secondary-button"
                disabled={disabled || historyBusy || !toVersion}
      ...
      <footer className="planning-review-footer">
        <button className="primary-button full-width" disabled={disabled || submitting} onClick={() => void approve()}>
          <CheckCircle2 size={15} />
          {submitting ? "提交中" : "批准并继续"}
        </button>
      </footer>
    </aside>
  );
