...
function ScoreGrid({ title, scores }: { title: string; scores: Record<string, number> }) {
  return (
    <section className="book-audit-section">
      <h3>{title}</h3>
...
export function BookAuditPanel({ report, totalChapters, disabled, onStartRevision }: Props) {
  ...
  return (
    <aside className="book-audit-panel">
      <header className="book-audit-header">
        <BookCheck size={20} />
        <div>
          <span>FINAL MANUSCRIPT AUDIT</span>
          <h2>全书终审</h2>
        </div>
        <strong>{report.overall_score.toFixed(1)}</strong>
      </header>
      {(report.judge_error || report.storage_error) && (
        <div className="book-audit-warning" role="status">
      ...
      {report.revision_priorities.length > 0 && (
        <section className="book-audit-section">
          ...
          <ol className="book-audit-priorities">
            {report.revision_priorities.map((item, index) => (
      ...
      <section className="book-audit-section findings">
        ...
        {report.findings.map((finding, index) => (
          <div className="book-audit-finding" key={`${finding.dimension}-${index}`}>
            <span>
              {SCORE_LABELS[finding.dimension || ""] ||
                finding.dimension ||
                (finding.source === "model" ? "文学评审" : "结构检查")}
            </span>
      ...
      <form className="book-revision-form" onSubmit={submit}>
        ...
        <div>
          <label>
            ...
            >
              {Array.from({ length: totalChapters }, (_, index) => index + 1).map((number) => (
