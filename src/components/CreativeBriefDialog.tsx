  ...
  return (
    ...
    >
      <section
        ref={dialogRef}
        tabIndex={-1}
        className={`model-settings-dialog creative-brief-dialog ${embedded ? "tool-page-panel" : ""}`}
      ...
      >
        <header className="model-settings-header">
          <div>
            ...
            <h2 id="creative-brief-title">创作约束</h2>
          </div>
          <div className="model-settings-header-actions">
            <span className="model-source-badge database">版本 v{version ?? 1}</span>
            <button
        ...
        <form onSubmit={submit}>
          <div className="creative-brief-dialog-content">
            ...
            <div className="brief-dialog-select-grid">
              <label>
                ...
                <select
                  ...
                  onChange={(event) => update("age_rating", event.target.value as CreativeBrief["age_rating"])}
                  disabled={busy}
                >
              ...
              <label>
                叙事视角
                <select
                  value={draft.point_of_view}
                  onChange={(event) => update("point_of_view", event.target.value as CreativeBrief["point_of_view"])}
                  disabled={busy}
                >
              ...
              <label>
                叙事时态
                <select
                  value={draft.narrative_tense}
                  onChange={(event) =>
                    update("narrative_tense", event.target.value as CreativeBrief["narrative_tense"])
                  }
                  disabled={busy}
                >
              ...
              <label>
                叙事距离
                <select
                  value={draft.narrative_distance}
                  onChange={(event) =>
                    update("narrative_distance", event.target.value as CreativeBrief["narrative_distance"])
                  }
                  disabled={busy}
                >
              ...
              <label>
                结局基调
                <select
                  value={draft.ending_tone}
                  onChange={(event) => update("ending_tone", event.target.value as CreativeBrief["ending_tone"])}
                  disabled={busy}
                >
            ...
            <fieldset className="brief-dialog-intensity">
              <legend>
              ...
              {(
                ...
                <label className="brief-dialog-slider" key={field}>
                  <span>
                    {label}
                    <output>{draft.intensity[field]}</output>
                  </span>
                  <input
                    type="range"
                    min="0"
            ...
            <label>
              ...
              <input
                ...
                placeholder="例如：降低黑暗度，改为开放式结局"
                disabled={busy}
              />
            </label>
            {versions.length > 0 && (
              <section className="creative-brief-history">
                <div className="block-label">
                  <Clock3 size={14} />
                  版本历史
                </div>
                {versions.slice(0, 6).map((item) => (
                  <div className="creative-brief-history-row" key={item.id}>
                    <strong>v{item.version_number}</strong>
                    <span>{item.change_summary || "未填写说明"}</span>
                    <time>{item.created_at.slice(0, 16).replace("T", " ")}</time>
                  </div>
                ))}
              </section>
            )}
          </div>
          <footer className="creative-brief-dialog-footer">
            <span>
              {disabled ? (
                <>
