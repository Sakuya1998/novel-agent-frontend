    ...
    <main className="workspace-overview">
      <header>
        <div>
          ...
          <h1>{workspace?.name ?? user?.tenant_name ?? "工作区"}</h1>
        </div>
        <button onClick={() => void logout().then(() => onNavigate("/login"))}>退出登录</button>
      </header>
      ...
      <button className="primary-button" onClick={() => onNavigate("/app")}>
        进入工作台
      </button>
    </main>
  );
