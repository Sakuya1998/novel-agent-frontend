...
export function RegisterPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await register(form);
      onNavigate("/app/workspaces/current/overview");
    } catch (reason) {
  ...
  return (
    <main className="auth-page">
      <form className="auth-form" onSubmit={submit} aria-labelledby="register-title">
        <span className="eyebrow">NOVEL AGENT</span>
        <h1 id="register-title">创建工作区</h1>
        <label>
          用户名
        ...
        <label>
          工作区名称
          <input value={form.tenant_name} onChange={update("tenant_name")} required />
        </label>
        <label>
          邮箱
