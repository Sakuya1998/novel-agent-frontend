...
export function SettingsPage({ section, onBack }: { section: SettingsSection; onBack: () => void }) {
  ...
  return (
    <main className="settings-page">
      <header>
        <button className="icon-button" title="返回工作台" aria-label="返回工作台" onClick={onBack}>
          <ArrowLeft size={17} />
        </button>
        <div>
      ...
      <section className="settings-placeholder">
        ...
        {empty ? (
          <>
            <span>请先选择一部作品后再进行导入导出。</span>
            <button className="secondary-button" onClick={onBack}>
              返回作品库
            </button>
          </>
        ) : (
          <span>页面入口已就绪，具体操作将在对应资源页完成。</span>
        )}
      </section>
      {section === "models" ? <ModelSettingsDialog open isStreaming={workbench.isStreaming} onClose={onBack} /> : null}
      {section === "audit" ? <MonitoringDialog open onClose={onBack} /> : null}
      {section === "resources" && !empty ? (
        <ImportExportDialog
          open
          novelTitle={workbench.novel?.title ?? ""}
          onClose={onBack}
