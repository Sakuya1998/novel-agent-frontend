...
import {
  compareChapterEvaluations,
  createNovel,
  deleteNovel,
  evaluateChapterVersion,
  evaluateMemoryQuality,
  exportNovel as exportNovelFile,
  ...
  getNovelState,
  getPlanningVersion,
  getPlanningVersionDiff,
  importNovel as importNovelFile,
  listCreativeBriefVersions,
  listEvaluationBenchmarks,
  listModelTraces,
  listNovels,
  rebuildMemory,
  runEvaluationBenchmark,
  setChapterEvaluationBaseline,
  startBookRevisionJob,
...
export function useWorkbench(enabled = true) {
  ...
  const refreshSelected = useCallback(async (id: string) => {
    ...
    if (typeof getNovelConflicts === "function") {
      try {
        ...
        setState((current) =>
          current?.novel_id === id ? { ...current, conflicts: conflicts?.issues ?? [] } : current,
        );
      } catch {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    refreshList()
      .catch((err: unknown) => setError(err instanceof Error ? err.message : "无法加载作品"))
  ...
  const loadMemoryQuality = useCallback(
    async (id = selectedId) => {
  ...
  const runMemoryQuality = useCallback(
    async (k = 5) => {
  ...
  const rebuildMemoryIndex = useCallback(
    async (evaluate = true, k = 5) => {
      if (!selectedId) throw new Error("尚未选择作品");
  ...
  const importNovel = useCallback(
    async (file: File, title = "", password = "") => {
  ...
  const resume = useCallback(
    async (review: ReviewSubmission | PlanningReviewSubmission) => {
      if (!selectedId) return;
      const id = selectedId;
      setError("");
      try {
  ...
  const startBookRevision = useCallback(
    async (chapterNumber: number, feedback: string) => {
      if (!selectedId) return;
      const id = selectedId;
      setError("");
      try {
  ...
  const generateCandidates = useCallback(
    async (count: number, instruction: string) => {
      if (!selectedId) return;
      const id = selectedId;
      setError("");
      try {
  ...
  const compareVersions = useCallback(
    async (fromVersion: number, toVersion: number) => {
      ...
      const result = await getChapterVersionDiff(selectedId, state.current_chapter, fromVersion, toVersion);
      return result.diff;
    },
    [selectedId, state],
  );
  ...
  const planningVersionContext = useCallback((): [PlanningArtifactType, number] => {
    ...
    return state.status === "blueprint_review" ? ["blueprint", 0] : ["scene", state.current_chapter];
  }, [state]);

  const loadPlanningVersion = useCallback(
  ...
  const comparePlanningVersions = useCallback(
    async (fromVersion: number, toVersion: number) => {
      if (!selectedId) return "";
      const [artifactType, chapterNumber] = planningVersionContext();
      const result = await getPlanningVersionDiff(selectedId, artifactType, chapterNumber, fromVersion, toVersion);
      return result.diff;
    },
    [planningVersionContext, selectedId],
  );
  ...
  const evaluateVersion = useCallback(
    async (versionNumber: number, includeJudge: boolean) => {
  ...
  const setEvaluationBaseline = useCallback(
  ...
  const compareEvaluations = useCallback(
    async (fromVersion: number, toVersion: number) => {
      if (!selectedId || !state) throw new Error("尚未选择章节");
  ...
  const updateCanon = useCallback(
    async (operation: CanonOperation) => {
      if (!selectedId) return;
      const id = selectedId;
      setError("");
      try {
  ...
  const updateBrief = useCallback(
    async (brief: CreativeBrief, changeSummary: string) => {
      if (!selectedId) throw new Error("尚未选择作品");
      const id = selectedId;
      setError("");
      try {
        const result = await updateCreativeBrief(id, brief, novel?.creative_brief_version, changeSummary);
  ...
  const addNovel = useCallback(
    async (payload: CreateNovelPayload) => {
      setError("");
